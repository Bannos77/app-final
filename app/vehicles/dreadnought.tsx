...
const FabiusBileCard = () => {
  ...
  const userId = '123';
  const itemId = 'redemptor-dreadnought';
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.imageContainer}>
        <Image
          source={require('@/assets/images/dreadnought.jpg')} // Local image file
          style={styles.image}
        />
      </View>
      ...
      <View style={styles.detailsContainer}>
        <View style={styles.titleRow}>
          <Text style={styles.name}>Redemptor Dreadnought</Text>
          <Text style={styles.points}>210 pts</Text>
        </View>

        <View style={styles.statTable}>
          {renderStatRow(['M', 'T', 'Sv', 'W', 'Ld', 'Oc'], true)}
          {renderStatRow(['8"', '10', '3+', '12', '6+', '4'])}
        </View>

        <Text style={styles.sectionTitle}>Ranged Weapons</Text>
        <Text style={styles.sectionName}>Heavy onslaught gatling cannon</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['24"', '12', '3+', '6', '0', '1'])}
        </View>
        <Text style={styles.sectionExtra}>DEVASTATING WOUNDS</Text>

        <Text style={styles.sectionName}>Icarus rocket pod</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['24"', 'D3', '3+', '8', '-1', '2'])}
        </View>
        <Text style={styles.sectionExtra}>ANTI-FLY 2+</Text>

        <Text style={styles.sectionName}>Macro plasma incinerator - standard</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['36"', 'D6+1', '3+', '8', '-3', '2'])}
        </View>
        <Text style={styles.sectionExtra}>BLAST</Text>
        
        <Text style={styles.sectionName}>Macro plasma incinerator - supercharge</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['36"', 'D6+1', '3+', '9', '-4', '3'])}
        </View>
        <Text style={styles.sectionExtra}>BLAST, HAZARDOUS</Text>

        <Text style={styles.sectionName}>Twin storm bolter</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['24"', '2', '3+', '4', '0', '1'])}
        </View>
        <Text style={styles.sectionExtra}>RAPID FIRE 2, TWIN-LINKED</Text>

        <Text style={styles.sectionTitle}>Melee Weapons</Text>
        <Text style={styles.sectionName}>Redemptor fist</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'WS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['Melee', '5', '3+', '12', '-2', '3'])}
        </View>
      </View>
    </ScrollView>
  );
