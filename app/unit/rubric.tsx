...
const FabiusBileCard = () => {
  ...
  const handlePress = async (route: '/(tabs)/settings') => {
    await hapticImpact();
    router.push(route);
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.imageContainer}>
        <Image
          source={require('@/assets/images/unit-rubric.jpg')}
          style={styles.image}
        />
      </View>
      ...
      <View style={styles.detailsContainer}>
        ...
        <Text style={styles.sectionTitle}>Soulreaper cannon</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['24"', '6', '3+', '6', '-1', '1'])}
        </View>
        <Text style={styles.sectionExtra}>DEVASTATING WOUNDS</Text>

        <Text style={styles.sectionTitle}>Warpsmite</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['18"', '2', '3+', '4', '-3', '1'])}
        </View>
        <Text style={styles.sectionExtra}>ANTI-INFANTRY 4+, DEVASTATING WOUNDS, PISTOL, PSYCHIC</Text>

        <Text style={styles.sectionTitle}>Warpflame pistol</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['12"', 'D6', 'N/A', '3', '-1', '1'])}
        </View>
        <Text style={styles.sectionExtra}>PISTOL, IGNORES COVER, TORRENT</Text>

        <Text style={styles.sectionTitle}>Warpflamer</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['12"', 'D6', 'N/A', '4', '-1', '1'])}
        </View>
        <Text style={styles.sectionExtra}>IGNORES COVER, TORRENT</Text>

        <Text style={styles.sectionTitle}>Close combat weapon</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'WS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['Melee', '2', '3+', '4', '0', '1'])}
        </View> 

        <Text style={styles.sectionName}>Force weapon</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['Melee', '3', '3+', '6', '-1', 'D3'])}
        </View>
