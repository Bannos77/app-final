...
const DaemonPrinceCard = () => {
  ...
  const userId = '123';
  const itemId = 'daemon-prince-card';
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.imageContainer}>
        <Image
          source={require('@/assets/images/daemon-prince.jpg')}
          style={styles.image}
        />
      </View>
      ...
      <View style={styles.detailsContainer}>
        <View style={styles.titleRow}>
          <Text style={styles.name}>Daemon Prince of Chaos</Text>
          <Text style={styles.points}>190 pts</Text>
        </View>

        <View style={styles.statTable}>
          {renderStatRow(['M', 'T', 'Sv', 'W', 'Ld', 'Oc'], true)}
          {renderStatRow(['8"', '10', '2+', '10', '6+', '3'])}
        </View>

        <Text style={styles.sectionTitle}>Ranged Weapons</Text>
        <Text style={styles.sectionName}>Infernal cannon</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'BS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['24"', '3', '2+', '5', '-1', '2'])}
        </View>

        <Text style={styles.sectionTitle}>Melee Weapons</Text>
        <Text style={styles.sectionName}>Hellforged weapons - strike</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'WS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['Melee', '6', '2+', '8', '-2', '3'])}
        </View>

        <Text style={styles.sectionName}>Hellforged weapons - sweep</Text>
        <View style={styles.weaponTable}>
          {renderWeaponRow(['Range', 'A', 'WS', 'S', 'AP', 'D'], true)}
          {renderWeaponRow(['Melee', '14', '2+', '6', '0', '1'])}
        </View>
      </View>
    </ScrollView>
  );
...
export default DaemonPrinceCard;
